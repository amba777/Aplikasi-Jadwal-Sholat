...
.header-section { text-align: center; color: #fafafa; padding: 20px 0; margin-bottom: 30px; }
.header-title { font-family: 'Poppins', sans-serif; font-size: 2.8rem; font-weight: 700; margin: 0 0 10px 0; text-shadow: 0 0 15px rgba(102, 126, 234, 0.5); }
...
.card { background: rgba(22, 27, 34, 0.7); backdrop-filter: blur(15px); border-radius: 20px; padding: 30px; border: 1px solid #30363d; box-shadow: 0 8px 32px 0 rgba(0, 0, 0, 0.37); height: 100%; display: flex; flex-direction: column; justify-content: center; align-items: center; text-align: center; transition: transform 0.3s ease, box-shadow 0.3s ease; }
.card:hover { transform: translateY(-5px); box-shadow: 0 12px 40px 0 rgba(0, 0, 0, 0.5); border-color: #58a6ff; }
.digital-clock { font-size: 5rem; font-weight: 700; color: #fafafa; margin-bottom: 15px; font-family: 'JetBrains Mono', monospace; text-shadow: 0 0 20px rgba(88, 166, 255, 0.6); }
...
.next-name { font-family: 'Poppins', sans-serif; font-size: 2.5rem; font-weight: 700; color: #58a6ff; margin: 10px 0; }
...
.prayer-card { background: rgba(22, 27, 34, 0.7); backdrop-filter: blur(10px); border-radius: 15px; padding: 25px 20px; text-align: center; border: 1px solid #30363d; transition: all 0.3s ease; color: #c9d1d9; }
.prayer-card:hover { transform: translateY(-8px); border-color: #58a6ff; box-shadow: 0 8px 24px rgba(88, 166, 255, 0.2); }
...
.prayer-time { font-size: 1.8rem; font-weight: 700; color: #58a6ff; font-family: 'JetBrains Mono', monospace; }
...
.azan-notification { background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; padding: 20px; border-radius: 15px; margin: 20px 0; text-align: center; font-weight: bold; font-size: 1.5rem; animation: pulse 2s infinite; box-shadow: 0 8px 32px rgba(102, 126, 234, 0.4); }
...
def find_next_prayer(prayer_times_dict):
    ...
    if not prayer_times_dict: 
        return "Subuh (Besok)", "00:00", "Memuat..."
    ...
    for key, nama in urutan_sholat:
        ...
        if waktu_sholat_str > sekarang.strftime("%H:%M"):
            ...
            waktu_sholat_berikutnya_str = waktu_sholat_str
            break
    else:
...
if data_sholat:
    ...
    <div class="main-grid">
        <div class="card">
            ...
            <div class="date-display">{data_sholat.get('gregorian_date_id', "...")}</div>
            <div class="hijri-date">{data_sholat.get("hijri_date", "...")}</div>
        </div>
        <div class="card">
